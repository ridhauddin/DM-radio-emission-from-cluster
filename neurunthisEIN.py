...
for y in range (1,11):
	...
	for k in range (1,13):
		...
		outfilename	='%sEIN%s.out'%(qproperties.cluster,channel[k-1])
############################### start: info table ###########################################################
...
#=================================================================================================
		def PAtR2(r,nuu):			# (intermediate)
			...
			nuu		= photon frequency	, in unit of GHz;
			PAtR2(r) 	: erg s^-1 Hz^-1 cm^-3'''
			PAtR2Y=qfunctions.AA() * qfunctions.ein_numdens_neu(r,m_neutralino,rhos,rs,n) * E_and_intQ[:,2]/ qfunctions.btot_sph(rayE,qfunctions.rayB(r,'radial'),r) * qfunctions.funcR(nuu,rayE,qfunctions.rayB(r,'radial')) * qfunctions.rayB(r,'radial')
			return np.trapz(PAtR2Y,x=rayEerg)
